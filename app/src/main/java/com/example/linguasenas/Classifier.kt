package com.example.imagepicker1
...
class Classifier(assetManager: AssetManager, modelPath: String, labelPath: String, inputSize: Int) {
    ...
    fun recognizeImageTopLabels(bitmap: Bitmap): String {
        val scaledBitmap = Bitmap.createScaledBitmap(bitmap, INPUT_SIZE, INPUT_SIZE, false)
        val byteBuffer = convertBitmapToByteBuffer(scaledBitmap)
        val result = Array(1) { FloatArray(lableList.size) }
        interpreter.run(byteBuffer, result)
        ...
        val sortedResults = result[0].withIndex().sortedByDescending { it.value }

        // Obtener la etiqueta con la mayor probabilidad (solo el primer elemento)
        val topLabel = lableList[sortedResults[0].index]
        return topLabel
    }
